...
public class BoardDAO {
	...
	public List<BoardVO> getSerch(String boardId, String text){
		...
		String sql =" SELECT * FROM (SELECT ROWNUM rn, BOARDNUM, BOARDREPLE, BOARDTITLE, BOARDID, BOARDTEAM, BOARDSCORE,BOARDHIREDATE FROM (SELECT BOARDNUM, BOARDREPLE, BOARDTITLE, BOARDID, BOARDTEAM, BOARDSCORE,BOARDHIREDATE FROM GAMEBOARD ORDER BY BOARDNUM DESC) where BOARDREPLE = 0)  where "+boardId+" like '%"+text+"%'";
		try {
			pstmt = conn.prepareStatement(sql);
